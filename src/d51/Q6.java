...
public class Q6 {
    public static void main(String[] args) {
        ...
        String sname = im.getItemName(im.MINOR, scode); // 商品コードから得た小分類の名称をItemMaster(im)から取得
...
class Item {
    ...
    Item(String code, String name){ // コンストラクタ。newされたときに呼び出され、メンバ変数を初期化
        this.code = code; // 引数からの値をメンバ変数に格納する際はthis.をつける
        this.name = name; // 同様、引数nameの値をthis.nameに格納
    }
    public String getCode(){ return code; } // codeの値を外部から取得するためのゲッターメソッド（メンバ変数への安全な窓口）
    public String getName(){ return name; } // nameの値を外部から取得するためのゲッターメソッド
}

// すべての項目を管理するとともに、大分類コードまたは小分類コードに対応する名称を返す処理を行うクラス。商品マスタとして用いる
class ItemMaster{
    ...
    ItemMaster(Item[] major, Item[] minor){ // このコンストラクタはオブジェクトを引数に呼び出されている
    ...
    // 大分類コードや小分類コードから名称をitemsから探して返す（ゲッターメソッド）
    public String getItemName(int codeLevel, String code){
        for(int i = 0; i < items[codeLevel].length; i++){ // 指定されたcodeLevel(大分類または小分類)の要素数分繰り返す
            if(code.equals(items[codeLevel][i].getCode())){ // 引数のcodeとitemsのcodeが同じ(equals)ならば
...
// コメントを記述したこのコードをPUSHして、URLをClassroomに提出しましょう